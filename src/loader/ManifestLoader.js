 ...
 */
 ...
 class ManifestLoader extends BaseLoader {
   ...
   parsePlaylist(source, callback) {
     const data = new ManifestParser(source)
   ...
   loadFile(segment, type, time) {
     ...
     this.httpWorker.onmessage = (event) => {
       ...
       } else if ((data.fileType === 'flv') && data.name === segment.no) {
